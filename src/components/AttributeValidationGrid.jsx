        ...
        <table className="w-full text-left text-sm text-slate-600">
          ...
          <tbody className="divide-y divide-slate-100">
            {keys.map((k) => {
              ...
              const isNormalized = !!normObj?.value;
              ...
              return (
                <tr key={k} className="hover:bg-blue-50/30 transition-colors">
                  <td className="px-6 py-3 font-medium text-slate-800">{k}</td>
                  <td className="px-6 py-3 font-mono text-xs">{raw}</td>
                  <td className="px-6 py-3">
                    {isChange ? (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-blue-50 text-blue-700 border border-blue-100 font-mono text-xs font-semibold shadow-sm">
                        <span className="text-blue-400">✨</span> {normLabel}
                      </span>
                    ) : (
                      <span className="text-slate-400 font-mono text-xs">{normLabel}</span>
                    )}
                  </td>
                </tr>
              );
