...
import { Zap, LayoutGrid, GitBranch, ShieldCheck, Download, X, Search, Sparkles, CheckCircle2, AlertCircle, Sun, Moon } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-blue-200 dark:selection:bg-blue-900">
      ...
      <div className="pt-6 pb-2 px-8 flex justify-between items-center max-w-[1400px] mx-auto">
        <div className="flex items-center gap-2">
          ...
          <span className="text-6xl font-black tracking-tighter text-slate-800 dark:text-slate-200">UNIHACK</span>
        </div>
        ...
        <div className="flex items-center gap-1 bg-white dark:bg-slate-900 p-1 rounded-full shadow-sm border border-slate-200 dark:border-slate-800">
          {["Batch Enrichment", "Single Pipeline"].map((nav) => (
            <button 
              ...
              className={`px-5 py-2 text-sm font-medium rounded-full transition-all ${activeTopNav === nav ? "bg-slate-800 dark:bg-slate-700 text-white shadow-md" : "text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"}`}
            >
              {nav}
            </button>
          ))}
      ...
      <main className="max-w-[1400px] mx-auto px-8 py-8">
        ...
        {activeTopNav === "Batch Enrichment" ? (
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-8 shadow-soft border border-slate-100 dark:border-slate-800">
            <BatchProcessor />
          </div>
        ) : (
          /* ── Main Dashboard Grid ── */
          ...
          <div className="space-y-6">
            ...
            {/* Upload Card */}
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 shadow-soft border border-slate-100 dark:border-slate-800 relative overflow-hidden">
              {/* Subtle background gradient blob */}
              <div className="absolute -top-20 -right-20 w-48 h-48 bg-blue-100 dark:bg-blue-900/20 rounded-full blur-3xl opacity-50 pointer-events-none"></div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] p-6 shadow-soft border border-slate-100 dark:border-slate-800">
              <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">Pipeline Execution</h2>
              <PipelineStepper phases={phases} jobStatus={jobStatus} />
            </div>
          </div>

          {/* RIGHT COLUMN: Results / Governance */}
          <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-soft border border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden relative">
            ...
            <div className="px-8 pt-8 pb-4 relative">
               <div className="relative group">
                 ...
                 <input 
                   ...
                   disabled={!jobId}
                   className="w-full pl-11 pr-4 py-3 bg-blue-50/50 dark:bg-slate-800 border border-blue-100 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-300 transition-all text-slate-700 dark:text-slate-200 placeholder:text-slate-400 disabled:opacity-50"
                 />
                 <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none">
                    <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-900 px-2 py-1 rounded shadow-sm border border-slate-200 dark:border-slate-700 uppercase tracking-widest">Enter to send</span>
                 </div>
               </div>

               {/* Chat Messages Overlay/Expansion */}
               {chatMessages.length > 0 && (
                 <div className="mt-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4 max-h-[300px] overflow-y-auto space-y-4 shadow-inner">
                   {chatMessages.map((msg, idx) => (
                     <div key={idx} className={`flex gap-3 text-sm ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                        ...
                        <div className={`p-3 rounded-2xl max-w-[80%] ${msg.role === "user" ? "bg-blue-500 text-white rounded-tr-none" : "bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-tl-none shadow-sm"}`}>
                          <div className="prose prose-sm prose-slate dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: msg.text.replace(/\n/g, '<br/>') }} />
                        </div>
                     </div>
                   ))}
                   {isChatting && chatMessages[chatMessages.length - 1]?.role === "user" && (
                     <div className="flex gap-3 text-sm flex-row">
                        ...
                        <div className="p-3 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 rounded-tl-none shadow-sm flex items-center gap-1">
            ...
            {/* Light Pill Tabs */}
            <div className="px-8 flex gap-2 border-b border-slate-100 dark:border-slate-800 pb-4 overflow-x-auto">
                ...
                >
                  ...
                  {tabBadge[id] && (
                    <span className={`w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold ml-1
                      ${tab === id ? "bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300" : "bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400"}`}>
                      {tabBadge[id]}
                    </span>
                  )}
                </button>
            ...
            {/* Tab Content Area */}
            <div className="flex-1 p-8 bg-slate-50/30 dark:bg-slate-900/50">
              {tab === "pipeline" && (
                <div className="h-full flex flex-col justify-center">
                  {!jobId && (
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                      ...
                      <h3 className="text-lg font-medium text-slate-800 dark:text-slate-200">Workspace Empty</h3>
                  ...
                  {jobId && pimData && (
                    <div className="space-y-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Pipeline Complete</h3>
                          <p className="text-sm text-slate-500 dark:text-slate-400">Successfully generated structured product intelligence.</p>
                        </div>
                      </div>
                      
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-5 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-sm">
                          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Generated Title</p>
                          <p className="text-slate-800 dark:text-slate-200 font-medium">{pimData.product?.short_title ?? "—"}</p>
                        </div>
                        <div className="p-5 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-sm">
                        ...
                        <div className="p-5 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-sm">
                          ...
                          <p className="font-mono text-blue-600 dark:text-blue-400">{pimData.classification?.unspsc?.code ?? "—"}</p>
                          <p className="text-sm text-slate-500 dark:text-slate-400 truncate">{pimData.classification?.unspsc?.commodity_name}</p>
                        </div>
                        <div className="p-5 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-sm">
                          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">ETIM Class</p>
                          <p className="font-mono text-cyan-600 dark:text-cyan-400">{pimData.classification?.etim?.class_code ?? "—"}</p>
                          <p className="text-sm text-slate-500 dark:text-slate-400 truncate">{pimData.classification?.etim?.class_name}</p>
                        </div>
                      </div>
                  ...
                  {complianceData && (
                    <>
                      <div className={`flex items-center gap-4 p-5 rounded-2xl border
                        ...
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${complianceData.overall_status === "PASS" ? "bg-emerald-100 dark:bg-emerald-900 text-emerald-600 dark:text-emerald-400" : "bg-amber-100 dark:bg-amber-900 text-amber-600 dark:text-amber-400"}`}>
                          {complianceData.overall_status === "PASS" ? <ShieldCheck className="w-6 h-6" /> : <AlertCircle className="w-6 h-6" />}
                        </div>
                        <div>
                          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Overall Status</p>
                          <p className={`text-xl font-bold ${complianceData.overall_status === "PASS" ? "text-emerald-700 dark:text-emerald-400" : "text-amber-700 dark:text-amber-400"}`}>
                      ...
                      <div className="space-y-3 mt-6">
                        {(complianceData.flags ?? []).map((f, i) => (
                          <div key={i}
                            className={`p-5 rounded-2xl border bg-white dark:bg-slate-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4
                            ...
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                <span className="font-semibold text-slate-800 dark:text-slate-200">{f.standard}</span>
                              </div>
                              <p className="text-slate-500 dark:text-slate-400 text-sm">{f.note}</p>
                            </div>
      ...
      {toast && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-4
          ...
          ${toast.type === "success" ? "bg-white dark:bg-slate-800 border-emerald-100 dark:border-emerald-800 text-slate-800 dark:text-slate-200"
